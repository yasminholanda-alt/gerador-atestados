...
# Upload do arquivo e campos de entrada
...
if doc_type == "Mídia (AP)":
    num_id = st.text_input("Número do PI:", placeholder="Ex: 37677")
else:
...
# Leitura e extração de dados do PDF
...
# Geração do atestado ao clicar no botão
if st.button("🚀 Gerar Atestado PDF", type="primary"):
    if not uploaded_file or not num_id:
        st.error("Por favor, anexe a AP/OC e preencha o número do PI/PP.")
    else:
        ...
        pdf.cell(60, 5, "EBM", ln=1, align="R")
        ...
        texto = f"Atestamos para fins de comprovação de execução de serviço prestados que para o cliente {dados['cliente_nome']}, CNPJ {dados['cliente_cnpj']}, intermediadas por essa agência de publicidade no período de acordo com as informações relacionadas abaixo."
